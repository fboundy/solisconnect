...
from .helpers import get_controller, redact_config, remove_entities_for_controller, set_controller, unique_id_generator
from .inverter_hub import SolisInverterHub
...
async def async_setup(hass: HomeAssistant, entry: ConfigEntry):
    ...
    async def service_set_time(call: ServiceCall) -> None:
        ...
        _LOGGER.error("⚠️ Entity with id %s not found in solisconnect TIME_ENTITIES", entity_id)
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    if not inverter_serial:
        pn_create(
            hass,
            "SolisConnect: Inverter Serial is missing. Please reconfigure the integration.",
            title="SolisConnect Configuration Issue",
            notification_id="solisconnect_missing_serial",
        )
    ...
    # Which protocol(s) feed this entry's entities. Pre-v4 entries are Modbus-only.
    protocol_mode = config.get(CONF_PROTOCOL_MODE, PROTO_MODBUS_ONLY)
    uses_modbus = protocol_mode != PROTO_CLOUD_ONLY
    uses_cloud = protocol_mode in (PROTO_CLOUD_ONLY, PROTO_BOTH_FAILOVER, PROTO_BOTH_MANUAL)
    ...
    _LOGGER.debug(redact_config(config))
    ...
    if uses_modbus:
        ...
        connection_id = "soliscloud"

    _LOGGER.info(f"Loaded SolisConnect Integration ({protocol_mode}/{connection_type}) with Model: {config.get('model')}")
    ...
    if inverter_template is None:
        pn_create(
            hass,
            "Your SolisConnect configuration is invalid. Please reconfigure the integration.",
            title="SolisConnect Configuration Issue",
            notification_id="solisconnect_invalid_config",
        )
    ...
    # The hub is what entities bind to; it delegates to the configured protocol controller(s).
    hub = SolisInverterHub(
        ...
        failover_primary=config.get(CONF_FAILOVER_PRIMARY),
    )
    ...
    if uses_modbus:
        ...
        hub.modbus = ModbusController(**controller_params)

    if uses_cloud:
        from homeassistant.helpers.aiohttp_client import async_get_clientsession

        from .cloud.api_client import SolisCloudApiClient
        from .cloud.cloud_controller import SolisCloudController

        api = SolisCloudApiClient(
            session=async_get_clientsession(hass),
            key_id=config[CONF_CLOUD_KEY_ID],
            key_secret=config[CONF_CLOUD_KEY_SECRET],
            username=config.get(CONF_CLOUD_USERNAME),
            password=config.get(CONF_CLOUD_PASSWORD),
        )
        hub.cloud = SolisCloudController(
            hass=hass,
            inverter_config=inverter_config,
            api=api,
            plant_id=config[CONF_CLOUD_PLANT_ID],
            serial_number=inverter_serial,
            identification=identification,
            poll_interval=config.get(CONF_CLOUD_POLL_INTERVAL, DEFAULT_CLOUD_POLL_INTERVAL),
            inverter_id=config.get(CONF_CLOUD_INVERTER_ID),
        )

    # Sensor groups and derived sensors bind to the hub (protocol-agnostic) and are handed
    # to each concrete controller so its retrieval loop knows what to poll.
    sensor_groups = []
    for group in sensors:
        ...
        sensor_groups.append(SolisSensorGroup(hass=hass, definition=group, controller=hub, identification=identification))
    ...
    derived_sensors = [
        SolisBaseSensor(
            ...
            unique_id=unique_id_generator(hub, entity.get("unique", "reserve")),
        )
        for entity in sensors_derived
    ]

    for protocol_controller in (hub.modbus, hub.cloud):
        if protocol_controller is not None:
            protocol_controller._sensor_groups = sensor_groups
            protocol_controller._derived_sensors = derived_sensors

    set_controller(hass, hub, entry)

    _LOGGER.debug(f"Config entry setup for {protocol_mode} ({connection_id}), slave {slave}")

    await hass.config_entries.async_forward_entry_setups(entry, [Platform.SENSOR])
    await hass.config_entries.async_forward_entry_setups(entry, PLATFORMS)

    # The hub owns retrieval lifecycle (it must stop/start retrievals on protocol switches).
    await hub.async_start()
    hass.data[DOMAIN].setdefault("data_retrieval", {})
    hass.data[DOMAIN]["data_retrieval"][entry.entry_id] = hub
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.version == 3:
        # v4 introduces protocol selection; existing entries keep talking Modbus only.
        new_data = {**config_entry.data}
        new_data.setdefault(CONF_PROTOCOL_MODE, PROTO_MODBUS_ONLY)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    # fails with "config entry for solisconnect.sensor has already been setup!".
    ...
    if unload_ok:
        ...
        if CONTROLLER in hass.data[DOMAIN]:
            ...
            if controller:
                ...
                # 3. Drop this entry's entities from the shared entity lists
                remove_entities_for_controller(hass, controller)
