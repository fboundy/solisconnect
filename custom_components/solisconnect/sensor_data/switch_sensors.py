from custom_components.solisconnect.data.enums import InverterFeature, InverterType


def _has_feature(inverter_config, feature: InverterFeature) -> bool:
    return feature in getattr(inverter_config, "features", [])


def get_switch_sensors(inverter_config):
    switch_sensors = [
        {
            "register": 90005,
            "entities": [
                {"bit_position": 0, "name": "SolisConnect Enabled"},
            ],
        }
    ]

    if inverter_config.type == InverterType.HYBRID:
        has_generator = _has_feature(inverter_config, InverterFeature.GENERATOR)
        has_ac_coupling = _has_feature(inverter_config, InverterFeature.AC_COUPLING)

        hybrid_sensors = [
            {
                "register": 43007,
                "entities": [
                    {"name": "Power State", "on_value": 190, "off_value": 222},
                ],
            },
            {
                "register": 43073,
                "entities": [
                    {"bit_position": 4, "name": "Grid feed in power limit switch"},
                ],
            },
            {
                "register": 43110,
                "entities": [
                    # Adheres to RS485_MODBUS ESINV-33000ID Hybrid Inverter V3.2 / Appendix 8
                    {"bit_position": 0, "name": "Mode - Self Use", "conflicts_with": [2, 6, 11], "control": True},
                    {"bit_position": 1, "name": "Mode - Timed Charge", "requires_any": [0, 6], "control": True},
                    {"bit_position": 2, "name": "Mode - Off-Grid", "conflicts_with": [0, 1, 6, 11], "control": True},
                    {"bit_position": 3, "name": "Mode - Battery Wake Up", "control": True},
                    {"bit_position": 4, "name": "Mode - Backup", "conflicts_with": [11], "control": True},
                    {"bit_position": 5, "name": "Mode - Grid Charge", "control": True},
                    {"bit_position": 6, "name": "Mode - Feed In Priority", "conflicts_with": [0, 11], "control": True},
                    {"bit_position": 7, "name": "Mode - Night OVD Retain", "control": True},
                    {"bit_position": 8, "name": "Battery Forcecharge Peakshaving", "enabled": False},
                    {"bit_position": 9, "name": "Battery Current Correction", "enabled": False},
                    {"bit_position": 10, "name": "Mode - Battery Condition", "control": True},
                    {"bit_position": 11, "name": "Mode - Peak Shaving", "conflicts_with": [0, 4, 6], "control": True},
                ],
            },
            {
                "register": 43483,
                "entities": [
                    {"bit_position": 0, "name": "Dual Backup Enable"},
                    {"bit_position": 2, "name": "Smart load port grid-connected forced output"},
                    {"bit_position": 3, "name": "Allow export switch under self-generation and self-use", "inverted": True},
                    {
                        "bit_position": 4,
                        "name": "Backup2Load manual/automatic switch (off = Manual, on = Automatic)",
                    },
                    {"bit_position": 5, "name": "Backup2Load manual enable"},
                    {"bit_position": 6, "name": "Smart load port stops output when off-grid"},
                    {"bit_position": 7, "name": "Grid Peak-shaving power enable"},
                ],
            },
            {
                "register": 43302,
                "entities": [
                    {
                        "bit_position": 0,
                        "name": "Boost not-generate-wave function (off control)",
                        "inverted": True,
                    },
                    {
                        "bit_position": 1,
                        "name": "DC injection adjustment function (off control)",
                        "inverted": True,
                    },
                    {"bit_position": 3, "name": "AFCI self-check"},
                    {
                        "bit_position": 4,
                        "name": "AFCI self-check mode (abnormal / arc test data)",
                    },
                    {
                        "bit_position": 8,
                        "name": "Hybrid grid connect with neutral (1PH L1-L2-N / 3PH L1-L2-L3-N)",
                        "inverted": True,
                    },
                    {
                        "bit_position": 9,
                        "name": "PV enable (Alpha all-in-one)",
                        "inverted": True,
                    },
                    {
                        "bit_position": 12,
                        "name": "MPPT Multi-peak scanning",
                    },
                    {
                        "bit_position": 13,
                        "name": "24-hour switch enable (S5 low-voltage energy storage)",
                    },
                    {"bit_position": 14, "name": "Daily PV insulation detection"},
                ],
            },
            {
                "register": 43249,
                "entities": [
                    {"bit_position": 0, "name": "MPPT Parallel Function"},
                    {"bit_position": 1, "name": "IgFollow"},
                    {"bit_position": 2, "name": "Relay Protection"},
                    {"bit_position": 3, "name": "I-Leak Protection"},
                    {"bit_position": 4, "name": "PV ISO Protection"},
                    {"bit_position": 5, "name": "Grid-Interference Protection"},
                    {"bit_position": 6, "name": "The DC component of the grid current protection switch"},
                    {"bit_position": 7, "name": "Const Voltage Mode Enable Const Voltage"},
                ],
            },
            {
                "register": 43135,
                "entities": [
                    {"name": "RC Force Battery Charge", "on_value": 1},
                    {"name": "RC Force Battery Discharge", "on_value": 2},
                ],
            },
            {
                "register": 43292,
                "entities": [
                    {"name": "Flexible Export Enabling Switch", "on_value": 170},
                ],
            },
            {
                # Register 44280 BIT04-BIT07 is a 4-bit "PV Control" field where
                # 0 = invalid (default, PV operates normally) and 1 = PV shutdown.
                # Setting bit_position=4 writes that nibble to value 1 when ON
                # and back to 0 when OFF, leaving the AC/battery port bits
                # (BIT00-BIT03) untouched.
                # NOTE: this register reverts to 0 after the RC Timeout
                # (register 43282, default 5 minutes) unless refreshed.
                "register": 44280,
                "entities": [
                    {"bit_position": 4, "name": "PV Shutdown"},
                ],
            },
            {
                "register": 43707,
                "entities": [
                    {"name": "Timed Charge Enable 1", "bit_position": 0, "control": True},
                    {"name": "Timed Charge Enable 2", "bit_position": 1, "control": True},
                    {"name": "Timed Charge Enable 3", "bit_position": 2, "control": True},
                    {"name": "Timed Charge Enable 4", "bit_position": 3, "control": True},
                    {"name": "Timed Charge Enable 5", "bit_position": 4, "control": True},
                    {"name": "Timed Charge Enable 6", "bit_position": 5, "control": True},
                    {"name": "Timed Charge Enable 7", "bit_position": 6, "control": True},
                    {"name": "Timed Charge Enable 8", "bit_position": 7, "control": True},
                    {"name": "Timed Charge Enable 9", "bit_position": 8, "control": True},
                    {"name": "Timed Charge Enable 10", "bit_position": 9, "control": True},
                    {"name": "Timed Charge Enable 11", "bit_position": 10, "control": True},
                    {"name": "Timed Charge Enable 12", "bit_position": 11, "control": True},
                ],
            },
        ]

        if has_generator or has_ac_coupling:
            port_entities = []
            if has_generator:
                port_entities.extend(
                    [
                        {"bit_position": 0, "name": "Generator connection position"},
                        {"bit_position": 1, "name": "With Generator"},
                        {"bit_position": 2, "name": "Generator enable signal"},
                        {"bit_position": 4, "name": "Generator access location"},
                    ]
                )
            if has_ac_coupling:
                port_entities.append({"bit_position": 3, "name": "AC Coupling Position (off = GEN port, on = Backup port)"})
            hybrid_sensors.append({"register": 43365, "entities": port_entities})

        if has_generator:
            hybrid_sensors.extend(
                [
                    {
                        "register": 43815,
                        "entities": [
                            {"bit_position": 0, "name": "Generator charging period 1 switch"},
                            {"bit_position": 1, "name": "Generator charging period 2 switch"},
                            {"bit_position": 2, "name": "Generator charging period 3 switch"},
                            {"bit_position": 3, "name": "Generator charging period 4 switch"},
                            {"bit_position": 4, "name": "Generator charging period 5 switch"},
                            {"bit_position": 5, "name": "Generator charging period 6 switch"},
                        ],
                    },
                    {
                        "register": 43340,
                        "entities": [
                            {"bit_position": 0, "name": "Generator Input Mode (off = Manual, on = Auto)"},
                            {"bit_position": 1, "name": "Generator Charge Enable"},
                        ],
                    },
                    {
                        "register": 43363,
                        "entities": [
                            {"name": "Force Start Generator", "on_value": 1},
                        ],
                    },
                ]
            )

        if has_ac_coupling:
            for sensor_group in hybrid_sensors:
                if sensor_group["register"] == 43483:
                    sensor_group["entities"].insert(1, {"bit_position": 1, "name": "AC Coupling Enable"})
                    break

        switch_sensors.extend(hybrid_sensors)
    elif inverter_config.type == InverterType.GRID or inverter_config.type == InverterType.STRING:
        switch_sensors.extend(
            [
                {
                    "read_register": 3089,
                    "write_register": 3069,
                    "entities": [
                        {"on_value": 0xAA, "off_value": 0x55, "name": "Enable power limit feature"},
                    ],
                }
            ]
        )

    # Add unique keys
    for main_entity in switch_sensors:
        register = main_entity.get("register", main_entity.get("read_register"))
        for child_entity in main_entity["entities"]:
            # Unique key format: switch_{register}_{bit_position or on_value}
            suffix = child_entity.get("bit_position")
            if suffix is None:
                suffix = child_entity.get("on_value")

            child_entity["unique"] = f"switch_{register}_{suffix}"

    return switch_sensors
